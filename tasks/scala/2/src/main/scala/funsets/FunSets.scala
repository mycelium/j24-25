...
object FunSets {
   ...
   * that satisfies `p`.
   */
    def exists(s: Set, p: Int => Boolean): Boolean = !forall(s, a => !p(a))  // проверяем, что не для всех чисел из `s` не выполняется p
                                                                             // (то есть существует такое, что для него выполняется)
