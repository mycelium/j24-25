...
public class SimpleJson {
    ...
    public static <T> T parseFromMap(Map<String, Object> map, Class<T> clazz){
    ...
    @SuppressWarnings("unchecked")
    private static <T> JsonDeserializer<T> instantiateDeserializer(
            Class<? extends JsonDeserializer<?>> deserializerClass
    ) {
        try {
            return (JsonDeserializer<T>) deserializerClass.getDeclaredConstructor().newInstance();
        } catch (Exception e) {
            throw new RuntimeException("Failed to instantiate deserializer", e);
        }
    }
    ...
    static <T> T convertValue(Object rawValue, Class<T> targetType) {
        ...
        // Проверяем наличие аннотации @JsonDeserialize
        JsonDeserialize annotation = targetType.getAnnotation(JsonDeserialize.class);
        if (annotation != null) {
            // Создаем экземпляр кастомного десериализатора
            JsonDeserializer<T> deserializer = instantiateDeserializer(annotation.using());
            return deserializer.deserialize((Map<String, Object>) rawValue);
        }
        ...
        if (isNumericType(targetType)){
            ...
            try {
                return (T) convertNumber(Double.parseDouble(rawValue.toString()), targetType);
        ...
        if (rawValue instanceof Map && !targetType.isPrimitive()) {
            try {
                ...
                for (Field field : getAllFields(targetType)) {
                    ...
                    Object valueInMap = map.get(fieldName);
                ...
                throw new RuntimeException("Error converting to " + targetType.getName(), e);
            }
        }
    ...
    static void serializeObject(Object obj, StringBuilder sb) {
        ...
        for (Field field : fields) {
            try {
                ...
                // Skip null values to match typical JSON behavior
                if (fieldValue == null) {
                    continue;
                }
