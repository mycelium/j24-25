...
public class LiJsonParser {
    ...
    public Object parseCommon() throws LiJsonParserException {
        ...
        //если json-массив
            ...
            throw new LiJsonParserException("Неправильный формат json: должен начинаться с '{' или '['");
        }
    }

    //метод для парсинга json-объекта в Map<String, Object>
    private Map<String, Object> parseJsObjectToMap() throws LiJsonParserException {
    ...
    private List<Object> parseJsArrayToListOfObjects() throws LiJsonParserException {
    ...
    public <T> T parseJsObjectToClass(Class<T> clazz) throws LiJsonParserException {
        ...
        throw new LiJsonParserException("Результат парсинга не является Map, его нельзя сопоставить с пользовательским классом");
    }

    private <T> T parseMapToClass(Map<String, Object> map, Class<T> clazz) throws LiJsonParserException {
        try {
            ...
            throw new LiJsonParserException("Ошибка при сопоставлении json-объекта с классом");
        }
    }

    //метод для определения типа значения и вызова соответствующего метода для парсинга
    private Object parseValue(String token) throws LiJsonParserException {
        switch (token) {
            case "{":
                return parseJsObjectToMap();
            case "[":
    ...
    private Number makeNumber(String token) throws LiJsonParserException {
        try {
            ...
            throw new LiJsonParserException("Неправильный формат числа: " + token);
        }
    }
}